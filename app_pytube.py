...
class DownloadThread(QThread):
    ...
    def run(self):
        try:
            ...
            # tqdm for watch progress
            with tqdm(total=stream.filesize, unit='B', unit_scale=True, ncols=100) as bar:
                def progress_function(chunk: bytes, file_handle):
                    bar.update(len(chunk))
                    self.progress_signal.emit(int(bar.n))

                stream.download(progress_callback=progress_function)
